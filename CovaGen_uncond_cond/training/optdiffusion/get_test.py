...
class PocketLigandPairDataset(Dataset):
    ...
    def __init__(self, raw_path, vae_path, save_path, transform=None):
        ...
        self.processed_path = os.path.join(save_path,
                                          'crossdocked_pocket10_processed_rnnattn256tanh.lmdb') # 这个就是lmdb那里那个db连接到的数据的位置！
...
#   LMDB提供 key-value 存储，其中每个键值对都是我们数据集中的一个样本。LMDB的主要作用是提供数据管理，可以将各种各样的原始数据转换为统一的key-value存储。
    ...
    def _process(self):
        ...
        # featurize
        vae = RNNAttn(load_fn=self.vae_path)   # 之前train的vae
        processed_data=[]
        fail=0
        success=0
...
if __name__ == '__main__':
    ...
    dataset = PocketLigandPairDataset('/workspace/datas/11/dataset/', # 这是 raw path
                                      vae_path='/workspace/codes/vaemodel/070_rnnattn-256_zinc.ckpt',
                                      save_path='/workspace/datas/11/dataset/processed')
    ...
    dataset,subsets = split(dataset,'/workspace/datas/11/dataset/crossdocked_pocket10/split_by_name.pt')
    train, val = subsets['train'], subsets['test']
    cnt = 0
    for key,item in val.dataset.name2id:
        if cnt == 163916:
            exp = key
        cnt+=1
